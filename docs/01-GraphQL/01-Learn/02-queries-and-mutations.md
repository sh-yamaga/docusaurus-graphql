...
先ほどの例では、文字列方を返す hero の name を問い合わせただけでしたが、フィールドにはオブジェクトを指定することもできます。ここでは、オブジェクトのサブセクションを作成してみましょう。GraphQLクエリは、関連するオブジェクトをフィールドに渡すことができるため、クライアントは、RESTアーキテクチャで必要となるような数回のラウンドトリップを行う代わりに、1回のリクエストで多くの関連データを取得することができます。
